...
class CameraSync {
  ...
  constructor(private map, private camera: THREE.Camera, private world = new THREE.Group()) {
  ...
  public dispose() {
    ...
    this.map.off("resize", () => this.setupCamera());
    this.updateCallback = () => {};
  }
  ...
  private makePerspectiveMatrix(fovy: number, aspect: number, near: number, far: number) {
...
class TileSet {
  public root: ThreeDeeTile;
  private url: string;

  constructor(private updateCallback: Function = () => { }) {
    this.url = null;
    this.root = null;
  }
  // TileSet.load
  public async load(url: string, styleParams: ThreeDStyleParams, requestOptions: RequestInit) {
    this.url = url;
    ...
    const refine = json.root.refine ? json.root.refine.toUpperCase() : "ADD";
    this.root = new ThreeDeeTile(json.root, resourcePath, styleParams, this.updateCallback, refine, requestOptions);
  }
}

class ThreeDeeTile {
  private loaded = false;
  public totalContent: THREE.Group;
  private tileContent: THREE.Group;
  private childContent: THREE.Group;
  private box: THREE.Box3;
  private debugColor: THREE.Color;
  private debugLine: THREE.LineSegments;
  private refine: string;
  private geometricError: number;
  private worldTransform: THREE.Matrix4;
  private transform: number[];
  private content: { uri?: string, url?: string; };
  private children: ThreeDeeTile[];
  private unloadedTileContent: boolean;
  private unloadedChildContent: boolean;
  private unloadedDebugContent: Boolean;

  constructor(json, private resourcePath, private styleParams: ThreeDStyleParams, private updateCallback, parentRefine, private requestOptions: RequestInit, parentTransform?: THREE.Matrix4) {
    ...
    const boundingVolume = json.boundingVolume;
    if (boundingVolume && boundingVolume.box) {
      let b = boundingVolume.box;
    ...
    if (json.children) {
      for (let i = 0; i < json.children.length; i++) {
        let child = new ThreeDeeTile(json.children[i], resourcePath, styleParams, updateCallback, this.refine, this.requestOptions, this.worldTransform);
      ...
      switch (type) {
        case "json":
          ...
          try {
            ...
            await tileset.load(url, this.styleParams, this.requestOptions);
            ...
            loader.parse(b3dmData.glbData, this.resourcePath, (gltf) => {
              ...
              gltf.scene.traverse(child => {
                if (child instanceof THREE.Mesh) {
                  ...
                  (child.material as THREE.Material).depthWrite = true; // necessary for Velsen dataset?
                }
              });
                ...
                gltf.scene.traverse(child => {
                  if (child instanceof THREE.Mesh) {
                    ...
                    if (this.styleParams.opacity != null) {
                      (child.material as THREE.Material).opacity = this.styleParams.opacity;
                      (child.material as THREE.Material).transparent = this.styleParams.opacity < 1.0 ? true : false;
                    }
                  }
                });
              }
              if (this.debugColor) {
                gltf.scene.traverse(child => {
                  if (child instanceof THREE.Mesh) {
                    (child.material as any).color = this.debugColor;
                  }
                });
              }
        ...
        case "pnts":
          try {
            ...
            if (this.styleParams.color) {
              material.vertexColors = false; //THREE.NoColors;
              ...
              material.vertexColors = true; //THREE.VertexColors;
  ...
  checkLoad(frustum: THREE.Frustum, cameraPosition: THREE.Vector3) {
...
class TileLoader {
  ...
  private version;
  private byteLength;
  protected featureTableJSON;
  protected featureTableBinary;
  public batchTableJson;
  private batchTableBinary;
  ...
  protected parseResponse(buffer: ArrayBuffer) {
    let header = new Uint32Array(buffer.slice(0, 28));
    let decoder = new TextDecoder();
    let magic = decoder.decode(new Uint8Array(buffer.slice(0, 4)));
    if (magic !== this.type) {
      throw new Error(`Invalid magic string, expected '${ this.type }', got '${ magic }'`);
    }
...
class Layer implements mapboxgl.CustomLayerInterface {
  ...
  private styleParams: ThreeDStyleParams;
  public loadStatus: number;
  private map: mapboxgl.Map;
  private camera: THREE.PerspectiveCamera;
  private scene: THREE.Scene;
  private mapQueryRenderedFeatures;
  private tileset: TileSet;
  private world: THREE.Group;
  private cameraSync: CameraSync;
  private outlinedObject;
  private renderer: THREE.WebGLRenderer;
  private raycaster: THREE.Raycaster;
  private outlineMesh: THREE.Mesh;
  private outlinePropertyIndex: number;
  private outlineIndex: number;

  constructor(params: ThreeDTileSetLayerParams) {
    if (!params) {
  ...
  onAdd(map: mapboxgl.Map, gl: WebGLRenderingContext) {
    ...
    this.tileset.load(this.url, this.styleParams, this.requestOptions).then(() => {
      if (this.tileset.root) {
  ...
  onRemove(map: mapboxgl.Map, gl: WebGLRenderingContext) {
    ...
    this.cameraSync.dispose();
    this.cameraSync = null;
  }
  queryRenderedFeatures(pointOrBox?: mapboxgl.PointLike | [mapboxgl.PointLike, mapboxgl.PointLike], options?): mapboxgl.MapboxGeoJSONFeature[] {
    let result = this.mapQueryRenderedFeatures(pointOrBox, options);
    if (!this.map || !(this.map as any).transform) {
      return result;
    }
    if (!(options && options.layers && !options.layers.includes(this.id))) {
      const geometry = pointOrBox as mapboxgl.Point;
      if (geometry && geometry.x && geometry.y) {
        var mouse = new THREE.Vector2();

        // // scale mouse pixel position to a percentage of the screen's width and height
        mouse.x = (geometry.x / (this.map as any).transform.width) * 2 - 1;
        mouse.y = 1 - (geometry.y / (this.map as any).transform.height) * 2;
        ...
        if (intersects.length) {
          ...
          if (intersect.object && (intersect.object as any).geometry &&
            (intersect.object as any).geometry.attributes &&
            (intersect.object as any).geometry.attributes._batchid) {
            let geometry = (intersect.object as any).geometry;
            ...
            if (keys.length) {
              ...
              (feature.properties as any).batchId = propertyIndex;
            }
          } else {
            if (intersect.index != null) {
              (feature.properties as any).index = intersect.index;
            } else {
              (feature.properties as any).name = this.id;
            }
          }
          if (options.outline !== false && (intersect.object !== this.outlinedObject ||
            ...
            if (intersect.object instanceof THREE.Mesh) {
              ...
              if (intersect.object &&
                ...
                (intersect.object.geometry as THREE.BufferGeometry).attributes &&
                (intersect.object.geometry as THREE.BufferGeometry).attributes._batchid) {
                // create new geometry from faces that have same _batchid
                let geometry = intersect.object.geometry as THREE.BufferGeometry;
                if (geometry.index) {
                  let ip1 = geometry.index.array[intersect.faceIndex * 3];
                  const data = (geometry.attributes._batchid as THREE.InterleavedBufferAttribute).data;
                  let idx = (geometry.attributes._batchid as THREE.InterleavedBufferAttribute).data.array[ip1 * 7 + 6];
                  let blockFaces = [];
                  for (let faceIndex = 0; faceIndex < geometry.index.array.length; faceIndex += 3) {
                    let p1 = geometry.index.array[faceIndex];
                    if (data.array[p1 * 7 + 6] === idx) {
                      let p2 = geometry.index.array[faceIndex + 1];
                      if (data.array[p2 * 7 + 6] === idx) {
                        let p3 = geometry.index.array[faceIndex + 2];
                        if (data.array[p3 * 7 + 6] === idx) {
                  ...
                  for (let vertexCount = 0, face = 0; face < blockFaces.length; face++) {
                    ...
                    let p2 = geometry.index.array[faceIndex + 1];
                    let p3 = geometry.index.array[faceIndex + 2];
                    let positions = (geometry.attributes.position as THREE.InterleavedBufferAttribute).data.array;
...
interface ThreeDTileSetLayerParams extends ThreeDStyleParams {
  id: string;
  url: string;
  requestOptions?: RequestInit;
}

interface ThreeDStyleParams {
  color?: number;
  opacity?: number;
  pointsize?: number;
}
